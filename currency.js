...
const setupEventListener = () => {
  ...
  formElement.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const convertionResult = Number(amountValue * rate).toFixed(2);
      convertResultElem.textContent = `${amountValue} ${fromCurrency.value} = ${convertionResult} ${toCurrency.value}`;
    } catch (error) {
